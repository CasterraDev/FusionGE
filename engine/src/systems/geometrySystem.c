...
geometry* geometrySystemAcquireById(u32 id) {
    if (id != INVALID_ID && systemPtr->registeredGeometries[id].geometry.id != INVALID_ID) {
    ...
    FERROR("geometrySystemAcquireById cannot load invalid geometry id. Returning nullptr.");
    return 0;
}
...
geometry* geometrySystemAcquireFromConfig(geometryConfig config, b8 autoRelease) {
    ...
    if (!g) {
        FERROR("Unable to obtain free slot for geometry. Adjust configuration to allow more space. Returning nullptr.");
        return 0;
    }
...
void geometrySystemRelease(geometry* geometry) {
    if (geometry && geometry->id != INVALID_ID) {
        ...
        if (ref->geometry.id == geometry->id) {
            ...
            FFATAL("Geometry id mismatch. Check registration logic, as this should never occur.");
        }
        return;
    }

    FWARN("geometrySystemAcquireById cannot release invalid geometry id. Nothing was done.");
}
...
geometry* geometrySystemGetDefault() {
    ...
    FFATAL("geometrySystemGetDefault called before system was initialized. Returning nullptr.");
    return 0;
}
...
b8 createGeometry(geometrySystemState* state, geometryConfig config, geometry* g) {
    ...
    if (strLength(config.materialName) > 0) {
...
void destroyGeometry(geometrySystemState* state, geometry* g) {
    ...
    if (g->material && strLength(g->material->name) > 0) {
...
geometryConfig geometrySystemGeneratePlaneConfig(f32 width, f32 height, u32 xSegmentCount, u32 ySegmentCount, f32 tileX, f32 tileY, const char* name, const char* materialName) {
    if (width == 0) {
    ...
    if (materialName && strLength(materialName) > 0) {
        ...
        strNCpy(config.materialName, DEFAULT_MATERIAL_NAME, GEOMETRY_NAME_MAX_LENGTH);
    }
