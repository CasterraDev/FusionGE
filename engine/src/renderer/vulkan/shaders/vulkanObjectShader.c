...
#define APPLICATION_SHADER_PREFIX "App.ObjectShader"
...
b8 vulkanShaderCreate(vulkanHeader* header, vulkanShader* outShader){
    ...
    VK_CHECK(vkCreateDescriptorSetLayout(header->device.logicalDevice, &layoutInfo, header->allocator, &outShader->globalDescriptorSetLayout));
    ...
    fzeroMemory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    ...
    VK_CHECK(vkCreateDescriptorSetLayout(header->device.logicalDevice, &layout_info, 0, &outShader->objectDescriptorSetLayout));
    ...
    objectPoolSizes[1].descriptorCount = localSamplerCnt * VULKAN_OBJECT_MAX_OBJECT_COUNT;
    ...
    VK_CHECK(vkCreateDescriptorPool(header->device.logicalDevice, &objectPoolInfo, header->allocator, &outShader->objectDescriptorPool));
...
void vulkanShaderDestroy(vulkanHeader* header, vulkanShader* shader){
    ...
    for (u32 i = 0; i < 2; i++){
        vkDestroyShaderModule(header->device.logicalDevice, header->objectShader.stages[i].module, header->allocator);
        header->objectShader.stages[i].module = 0;
    }
}
...
void vulkanShaderApplyMaterial(vulkanHeader* header, struct vulkanShader* shader, material* material){
    ...
    vulkanObjectShaderState* objState = &shader->objectStates[material->shaderID];
    ...
    fzeroMemory(descriptorWrites, sizeof(VkWriteDescriptorSet) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
...
b8 vulkanShaderResourceAcquire(vulkanHeader* header, struct vulkanShader* shader, material* mat) {
    // TODO: free list
    mat->shaderID = shader->objectUniformBufferIdx;
    shader->objectUniformBufferIdx++;

    vulkanObjectShaderState* object_state = &shader->objectStates[mat->shaderID];
    for (u32 i = 0; i < VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT; ++i) {
        for (u32 j = 0; j < 3; ++j) {
...
void vulkanShaderResourceRelease(vulkanHeader* header, struct vulkanShader* shader, material* mat) {
    vulkanObjectShaderState* object_state = &shader->objectStates[mat->shaderID];
