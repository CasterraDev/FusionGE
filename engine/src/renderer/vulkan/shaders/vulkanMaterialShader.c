...
#define APPLICATION_SHADER_PREFIX "App.MaterialShader"

b8 vulkanMaterialShaderCreate(vulkanHeader* header, vulkanOverallShader* outShader){
    ...
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i) {
        if (!createShaderModule(header, APPLICATION_SHADER_PREFIX,
            stage_type_strs[i], stage_types[i], i, outShader->stages)){
            FERROR("Failed to create %s shader: %s", stage_type_strs[i], APPLICATION_SHADER_PREFIX);
            return false;
        }
    }
    ...
    VULKANSUCCESS(vkCreateDescriptorSetLayout(header->device.logicalDevice, &layoutInfo, header->allocator, &outShader->globalDescriptorSetLayout));
    ...
    if (vkCreateDescriptorPool(header->device.logicalDevice, &globalPoolInfo, header->allocator, &outShader->globalDescriptorPool) != VK_SUCCESS) {
    ...
    VkDescriptorSetLayoutBinding bindings[VULKAN_OVERALL_SHADER_DESCRIPTOR_COUNT];
    fzeroMemory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_OVERALL_SHADER_DESCRIPTOR_COUNT);
    ...
    VULKANSUCCESS(vkCreateDescriptorSetLayout(header->device.logicalDevice, &layout_info, 0, &outShader->objectDescriptorSetLayout));
    ...
    objectPoolSizes[1].descriptorCount = localSamplerCnt * VULKAN_OVERALL_MAX_OBJECT_COUNT;
    ...
    VULKANSUCCESS(vkCreateDescriptorPool(header->device.logicalDevice, &objectPoolInfo, header->allocator, &outShader->objectDescriptorPool));
    ...
    config.name = "Object Graphics Pipeline";
    ...
    vulkanBufferCreate(header, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, true, &outShader->globalUniformBuffer);
    ...
    if (vkAllocateDescriptorSets(header->device.logicalDevice, &allocInfo, outShader->globalDescriptorSets) != VK_SUCCESS) {
    ...
    if (!vulkanBufferCreate(
            header,
            sizeof(materialUBO) * MAX_MATERIAL_COUNT,
...
void vulkanMaterialShaderDestroy(vulkanHeader* header, vulkanOverallShader* shader){
    vkDestroyDescriptorPool(header->device.logicalDevice, shader->objectDescriptorPool, header->allocator);
    vkDestroyDescriptorSetLayout(header->device.logicalDevice, shader->objectDescriptorSetLayout, header->allocator);
    ...
    vkDestroyDescriptorPool(header->device.logicalDevice, shader->globalDescriptorPool, header->allocator);
...
void vulkanMaterialShaderUse(vulkanHeader* header, vulkanOverallShader* shader){
    vulkanPipelineBind(&header->graphicsCommandBuffers[header->imageIdx], VK_PIPELINE_BIND_POINT_GRAPHICS, &shader->pipeline);
}

void vulkanMaterialShaderUpdateGlobalState(vulkanHeader* header, vulkanOverallShader* shader){
    ...
    vulkanBufferLoadData(header, &shader->globalUniformBuffer, offset, range, 0, &shader->globalUbo);
    ...
    VkWriteDescriptorSet newDescriptor = {VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET};
    ...
    vkUpdateDescriptorSets(header->device.logicalDevice, 1, &newDescriptor, 0, 0);
}

void vulkanMaterialShaderSetModel(vulkanHeader* header, struct vulkanOverallShader* shader, mat4 model){
    u32 imgIdx = header->imageIdx;
    vulkanCommandBuffer cb = header->graphicsCommandBuffers[imgIdx];
    vkCmdPushConstants(cb.handle, shader->pipeline.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(mat4), &model);
}

void vulkanMaterialShaderApplyMaterial(vulkanHeader* header, struct vulkanOverallShader* shader, material* material){
    u32 imgIdx = header->imageIdx;
    vulkanCommandBuffer cb = header->graphicsCommandBuffers[imgIdx];

    vulkanOverallShaderState* objState = &shader->objectStates[material->shaderID];
    VkDescriptorSet objSet = objState->descriptorSets[imgIdx];

    VkWriteDescriptorSet descriptorWrites[VULKAN_OVERALL_SHADER_DESCRIPTOR_COUNT];
    fzeroMemory(descriptorWrites, sizeof(VkWriteDescriptorSet) * VULKAN_OVERALL_SHADER_DESCRIPTOR_COUNT);
    ...
    vulkanBufferLoadData(header, &shader->objectUniformBuffer, offset, range, 0, &obo);

    // Only do this if the descriptor has not yet been updated.
    if (objState->descriptorStates[descriptorIdx].generations[imgIdx] == INVALID_ID) {
    ...
    for (u32 samplerIdx = 0; samplerIdx < samplerMaxCnt; ++samplerIdx) {
        ...
        u32* descriptorGeneration = &objState->descriptorStates[descriptorIdx].generations[imgIdx];
        u32* descriptorID = &objState->descriptorStates[descriptorIdx].ids[imgIdx];
        ...
        // If the descriptorID is not the same then the texture was swapped for a different one.
        // If the generations don't match then the texture/material was updated
        if (t && (*descriptorID != t->id || *descriptorGeneration != t->generation || *descriptorGeneration == INVALID_ID)) {
            ...
            imgInfos[samplerIdx].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    ...
    if (descriptorUpdatedCnt > 0) {
        vkUpdateDescriptorSets(header->device.logicalDevice, descriptorUpdatedCnt, descriptorWrites, 0, 0);
    }

    // Bind the descriptor set to be updated, or in case the shader changed.
    vkCmdBindDescriptorSets(cb.handle, VK_PIPELINE_BIND_POINT_GRAPHICS, shader->pipeline.pipelineLayout, 1, 1, &objSet, 0, 0);
}

b8 vulkanMaterialShaderResourceAcquire(vulkanHeader* header, struct vulkanOverallShader* shader, material* mat) {
    // TODO: free list
    mat->shaderID = shader->objectUniformBufferIdx;
    shader->objectUniformBufferIdx++;

    vulkanOverallShaderState* object_state = &shader->objectStates[mat->shaderID];
    for (u32 i = 0; i < VULKAN_OVERALL_SHADER_DESCRIPTOR_COUNT; ++i) {
        for (u32 j = 0; j < 3; ++j) {
    ...
    VkResult result = vkAllocateDescriptorSets(header->device.logicalDevice, &alloc_info, object_state->descriptorSets);
...
void vulkanMaterialShaderResourceRelease(vulkanHeader* header, struct vulkanOverallShader* shader, material* mat) {
    vulkanOverallShaderState* object_state = &shader->objectStates[mat->shaderID];

    const u32 descriptor_set_count = 3;
    vkDeviceWaitIdle(header->device.logicalDevice);
    // Release object descriptor sets.
    VkResult result = vkFreeDescriptorSets(header->device.logicalDevice, shader->objectDescriptorPool, descriptor_set_count, object_state->descriptorSets);
    if (result != VK_SUCCESS) {
