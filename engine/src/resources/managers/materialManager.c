...
b8 materialManagerLoad(resourceManager* self, const char* name, resource* outResource){
    ...
    outResource->fullPath = strDuplicate(fileLocation);
    ...
    while(fsReadLine(&f, 511, &c, &lineLen)){
        ...
        if (strEqualI(tvar, "Name")){
            ...
            resmat->diffuseMap.texture = textureSystemTextureGetCreate(tval, true);
...
void materialManagerUnload(resourceManager* self, resource* resource){
    ...
    u32 pathLen = strLength(resource->fullPath);
    if (pathLen){
        ffree(resource->fullPath, sizeof(char) * pathLen + 1, MEMORY_TAG_STRING);
    }
