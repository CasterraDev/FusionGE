...
b8 shaderManagerLoad(resourceManager* self, const char* name,
    ...
    shaderConfig* r = fallocate(sizeof(shaderConfig), MEMORY_TAG_RESOURCE);
    r->name = 0;
...
void shaderManagerUnload(resourceManager* self, resource* resource) {
    shaderConfig* r = (shaderConfig*)resource->data;
    ...
    fzeroMemory(r, sizeof(shaderConfig));
