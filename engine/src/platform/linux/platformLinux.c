...
b8 platformStartup(u64* memoryRequirement, void* state, const char* appName,
    ...
    *memoryRequirement = sizeof(platformState);
    ...
    u32 value_list[] = {systemPtr->screen->black_pixel, event_values};

    // Create the window
    xcb_void_cookie_t cookie = xcb_create_window(
        systemPtr->connection,
        ...
        systemPtr->screen->root_visual, event_mask, value_list);
...
b8 platformPumpMessages() {
    ...
    while (event != 0) {
        ...
        case XCB_CONFIGURE_NOTIFY: {
            ...
            // but should be
            //  passed anyway since a change in the x/y could mean an upper-left
            //  resize.
            // The app layer can decide what to do with it.
            xcb_configure_notify_event_t* configure_event =
                (xcb_configure_notify_event_t*)event;
            // Fire an event. the app layer should pick this up, but not handle
            // it
